...
function ExploreDropdown({ path }: { path: string }) {
  ...
  const dropActive = ['/search', '/saved', '/post-job', '/resume'].includes(path)
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={() => setOpen(o => !o)} style={{
        ...
        background: dropActive ? 'rgba(0,201,177,0.1)' : 'transparent',
        color: dropActive ? '#00C9B1' : '#8B9DC3',
        border: dropActive ? '1px solid rgba(0,201,177,0.2)' : '1px solid transparent',
        transition: 'all 0.18s',
      }}>
        <FileText size={15} />
        <span>Explore</span>
      ...
      {open && (
        <div style={{
          ...
          {/* Resume Score */}
          <Link href="/resume" onClick={() => setOpen(false)} style={{
          ...
          >
            <div style={{ width: 34, height: 34, borderRadius: 9, background: 'rgba(0,201,177,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <FileText size={16} color="#00C9B1" />
            </div>
            <div>
              <div style={{ fontSize: 14, fontWeight: 600, color: path === '/resume' ? '#00C9B1' : '#F0F4FF' }}>Resume Score</div>
              <div style={{ fontSize: 12, color: '#4A6FA5', marginTop: 1 }}>AI-powered ATS analysis</div>
            </div>
          </Link>
      ...
      {open && (
        <div style={{
          ...
          <div style={{ padding: '14px 16px', borderBottom: '1px solid #1E3A5F' }}>
            ...
            <div style={{ fontSize: 12, color: '#4A6FA5', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user.email}</div>
          </div>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      <Link href="/signin" style={{
        ...
        color: '#8B9DC3', transition: 'all 0.18s',
      }}
        onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.borderColor = '#00C9B1'; (e.currentTarget as HTMLAnchorElement).style.color = '#00C9B1' }}
        onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.borderColor = '#1E3A5F'; (e.currentTarget as HTMLAnchorElement).style.color = '#8B9DC3' }}
      >
      ...
      <Link href="/signup" style={{
        ...
        color: '#0A1628', boxShadow: '0 0 14px rgba(0,201,177,0.3)',
    ...
    <nav style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          {/* Auth section — skeleton while hydrating to avoid flash */}
          {!isLoading && (
